...
echo "2. Go to the 'Amazon RDS/Snapshots/System' and find the latest snapshot for the database 'reinvest-$NODE_ENV-postgresql'"
echo "3. Copy the snapshot ARN"
echo "4. Enter the ARN below and click 'Enter'"
echo "5. Wait for the database to be restored"
echo "6. Go to the 'AWS/Cloudformation/Stacks/reinvest-${NODE_ENV}/Outputs. Find 'DatabaseHost' variable. Copy the value and replace Github Actions '${NODE_ENV}_POSTGRESQL_HOST' variable with it"
echo "7. Run the functions pipeline to deploy the functions with the new database host"
echo "8. Optional - you can remove previous database instance to save costs. Go to the 'Amazon RDS/Instances' and find the previous database instance."
...
echo "Database restore procedure completed. Validate manually if the database is restored correctly. Do not forget of steps 6-8!"
