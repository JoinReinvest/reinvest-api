...
import {
    DATABASE_CONFIG,
    SQS_CONFIG,
    COGNITO_CONFIG, NORTH_CAPITAL_CONFIG, VERTALO_CONFIG
...
const northCapitalRouter = () => {
    ...
    router.post("/get-profile", async (req: any, res: any) => {
        try {
            ...
            const mappedRecord = await databaseProvider.provide()
...
const vertaloRouter = () => {
    ...
    router.post("/get-account", async (req: any, res: any) => {
        try {
            ...
            const vertaloSyncRecord = await databaseProvider.provide()
