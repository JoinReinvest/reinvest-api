  ...
  () => {
    ...
    describe('When I want to buy shares', async () => {
      ...
      it('Then I should be able to create a trade', async () => {
        tradeId = await requester.createTrade(offeringId, accountId, 'ACH', shares, '10.0.0.1');
      ...
      it('And Then I should be able to check the details of my trade', async () => {
        const tradeHistory = await requester.getTradeHistory(accountId, tradeId);
      ...
      it('And Then I should NOT be able to transfer my funds with the same trade twice', async () => {
        ...
        await expectThrowsAsync(() => request, 'Request error: [210] ACH request already sent this trade.');
      });

      it('And Then I should be able to check the status of my payment', async () => {
        const status = await requester.getExternalFundMoveInfo(accountId, fundsTransferRefNumber);

        expect(status).is.equal('Pending');
      });

      it('And Then I should be able to check the details of my payment', async () => {
        const details = await requester.getExternalFundMoveDetails(accountId, fundsTransferRefNumber);
      ...
      it('And Then I should be able to get all ACH Pending transactions', async () => {
        const listOfTransactions = await requester.getAchPendingTransactionsForAccount(accountId);
