...
context('Given I am an issuer and I am an admin', () => {
  ...
  describe('When I have created offer without escrow account', () => {
    ...
    it('Then I should not be able to get the escrow account for the offering if offering not exists', async () => {
      const request = requester.getOfferingEscrowAccount('123');
    ...
    it('Then I should be able to create an escrow account', async () => {
      const escrowAccountStatus = await requester.createEscrowAccount(offeringId, '110.000000', 'Test Bank Name', '123456789', 'Test Escrow Account');
    ...
    it('And Then I should be able to get the escrow account for the offering', async () => {
      const escrowAccount = await requester.getOfferingEscrowAccount(offeringId);
      expect(escrowAccount).contains.keys('RoutingNumber', 'AccountNumber', 'escrowAccountStatus');
    });
  });
  ...
  describe('When I have created offer with an escrow account', () => {
    ...
    it('Then I should not be able to create an escrow account', async () => {
      const createEscrowAccountRequest = requester.createEscrowAccount(offeringId, '110.000000', 'Test Bank Name', '123456789', 'Test Escrow Account');

      await expectThrowsAsync(() => createEscrowAccountRequest, 'Request error: [115] This offer already has finance escrow account.');
    });
  });
});
