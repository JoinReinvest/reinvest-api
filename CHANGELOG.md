...
- split concepts of NAV and Unit Price
  - NAV concept is used by Shares and Dividends module
  - Unit Price concept is used by Investments and Trading modules
- Admin API updates:
  - removed `synchronizePortfolioNav` mutation
  - added `synchronizePortfolioUnitPrice` mutation - it synchronize unit price for a portfolio from North Capital offering
  - added `setPortfolioNav` mutation - it allows to manually set NAV for a portfolio (admin must provide NAV value and number of shares)
  - updated `registerPortfolio` mutation - it requires to provide initial NAV and number of shares for a new/first portfolio
  - updated `getPortfolioDetails` query - it returns also fields current Unit Price and unit prices history list
  - update `Nav` type:
    - updated field `dateSynchronization` to `dateUpdated`
    - updated field `unitPrice` to `unitNav`
  - new type `UnitPrice` - it represents unit price for a portfolio
- User API updates:
  - updated `Nav` type - updated field `unitPrice` to `unitNav`
