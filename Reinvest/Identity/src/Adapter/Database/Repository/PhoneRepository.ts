...
export class PhoneRepository {
  ...
  public async storeToken(oneTimeToken: OneTimeToken): Promise<boolean> {
    ...
    return this.transactionalAdapter.transaction(`Sending TOPT to phone for user ${userId}`, async () => {
  ...
  public async findToken(userId: string, phone: PhoneNumber): Promise<OneTimeToken | never> {
    const { topt, createdAt, expiresAfterMinutes, tries } = await this.databaseAdapterProvider
      .provide()
  ...
  private async updateToken(userId: string, phoneNumber: string): Promise<boolean> {
    return this.transactionalAdapter.transaction(`Saving phone number verification for user ${userId}`, async () => {
      await this.removeToken(userId);
  ...
  private async removeToken(userId: string): Promise<void> {
    await this.databaseAdapterProvider.provide().deleteFrom(phoneVerificationTable).where('userId', '=', userId).execute();
  }

  private async update(userId: string, toUpdate: UpdateablePhoneCodeRow): Promise<void> {
    await this.databaseAdapterProvider.provide().updateTable(phoneVerificationTable).set(toUpdate).where('userId', '=', userId).execute();
  }
}
