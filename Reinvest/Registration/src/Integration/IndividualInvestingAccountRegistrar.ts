...
class IndividualInvestingAccountRegistrar {
    ...
    public register(profileId: ProfileId, investingAccountId: InvestingAccountId): Result {
        try {
            const mainPartyId = this.mappingQuery.getStakeholderPartyId(profileId);

            const accountId = this.northCapitalAccountRegistrarAdapter.registerAccountForParty(mainPartyId);
