...
class BeneficiaryInvestingAccountRegistrar {
    ...
    public register(profileId: ProfileId, ownerInvestingAccountId: InvestingAccountId, beneficiaryInvestingAccountId: InvestingAccountId): Result {
        try {
            const accountId = this.mappingQuery.getNCAccountForInvestingAccount(ownerInvestingAccountId);

            this.mappingRepository.mapInvestingAccountToNCAccount(beneficiaryInvestingAccountId, accountId);

            const investorId = this.vertaloInvestorRegistrarAdapter.registerInvestorAccount(profileId, beneficiaryInvestingAccountId)
            this.mappingRepository.mapInvestingAccountToInvestor(beneficiaryInvestingAccountId, investorId);
        } catch (error: any) {
