...
import { northCapitalDocumentsSynchronizationTable } from 'Registration/Adapter/Database/DatabaseAdapter';
...
export async function up(db: Kysely<LegalEntitiesDatabase>): Promise<void> {
  await db.schema
    ...
    .addColumn('documentId', 'uuid', col => col.primaryKey().unique().notNull())
