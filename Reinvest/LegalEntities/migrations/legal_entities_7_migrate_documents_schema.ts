...
async function getProfileDocuments(db: Kysely<LegalEntitiesDatabase>): Promise<{ idScan: any; profileId: string }[]> {
  const data = await db.selectFrom('legal_entities_profile').select(['profileId', 'idScan']).where('idScan', 'is not', null).execute();
