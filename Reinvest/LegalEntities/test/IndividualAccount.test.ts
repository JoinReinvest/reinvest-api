...
import { NetIncome, NetWorth, ValueRangeInput } from 'Reinvest/LegalEntities/src/Domain/ValueObject/ValueRange';
...
context('Given an investor created an individual draft account', () => {
  ...
  describe('When the investor wants to complete draft', async () => {
    ...
    it('Or add an avatar without details Then expects validation error', async () => {
      ...
      expectThrows(() => draftAccount.setAvatar(Avatar.create(input)), 'avatar:MISSING_MANDATORY_FIELDS');
    });
