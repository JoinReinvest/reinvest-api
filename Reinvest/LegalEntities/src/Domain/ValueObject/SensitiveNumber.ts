...
export abstract class SensitiveNumber implements ToObject {
  ...
  protected static createFromSchema(
    sensitiveNumber: SensitiveNumberSchema | string,
    validationStrategy: ValidationStrategy,
    anonymizationStrategy: AnonymizationStrategy,
  ): SensitiveNumberSchema {
    try {
      if (typeof sensitiveNumber === 'string') {
  ...
  protected static createFromRawValue(
...
export class SSN extends SensitiveNumber {
  ...
  static createFromRawSSN(sensitiveNumber: string): SSN {
    const { anonymized, encrypted, hashed } = super.createFromRawValue(sensitiveNumber, SSN.validateSSN, SSN.anonymize);
...
export class EIN extends SensitiveNumber {
  ...
  static createFromRawEIN(sensitiveNumber: string): EIN {
    const { anonymized, encrypted, hashed } = super.createFromRawValue(sensitiveNumber, EIN.validateEIN, EIN.anonymize);
