...
export class CommonTransaction implements Transaction {
  ...
  protected waitForAdminManualAction(
    reason: ManualActionReason
  ): TransactionDecision {
    return new TransactionDecision(
      WaitForAdminManualAction.create(this.transactionId),
      TransactionStateChange.waitingForAdminManualAction(
        this.transactionId,
        reason
      )
    );
  ...
  protected cancelTrade(
    reason: FailureCompletionReason = FailureCompletionReason.TransactionCancelledManually
  ) {
    return new TransactionDecision(
