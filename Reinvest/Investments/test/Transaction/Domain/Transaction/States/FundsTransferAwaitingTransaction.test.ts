...
context(
  "Given the subscription agreement was signed and awaiting for a funds transfer initialization",
  () => {
    const transactionId = new TransactionId("123456");
    const transaction = new FundsTransferAwaitingTransaction(
      transactionId,
      Counter.init()
    );

    describe("When the system initializes the funds transfer", () => {
      const fundsTransferInitiatedEvent = new FundsTransferInitialized(
        transactionId
      );
    ...
    describe("When the funds transfer failed", () => {
      const fundsTransferFailed = new FundsTransferInitializationFailed(
        transactionId
      );

      it("Then the transaction should be cancelled", async () => {
        const decision: TransactionDecision =
          transaction.execute(fundsTransferFailed);
