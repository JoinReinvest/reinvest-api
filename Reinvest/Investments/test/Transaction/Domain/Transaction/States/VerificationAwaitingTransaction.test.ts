...
import { VerificationAwaitingTransaction } from "../../../../../src/Transaction/Domain/States/VerificationAwaitingTransaction";
...
context(
  "Given the payment was successful and awaiting for the verification of the investor and investment",
  () => {
    ...
    describe("When the grace period ended", () => {
      ...
      it("Then the transaction should note it and waits for other verifications", async () => {
        const decision: TransactionDecision =
          transaction.execute(gracePeriodEnded);

        expect(decision.command).is.instanceof(DoNothing);
        expect(decision.stateChange.status).is.equal(TransactionState.Same);
        expect(decision.stateChange.metadata.gracePeriodStatus).is.equal(
          GracePeriodStatus.Ended
        );
        expect(
          decision.stateChange.metadata.investorVerificationStatus
        ).is.equal(InvestorVerificationStatus.Unverified);
        expect(
          decision.stateChange.metadata.investmentVerificationStatus
        ).is.equal(InvestmentVerificationStatus.Unverified);
      });
    });
    ...
    describe("And when the investor is verified", () => {
      ...
      it("Then the transaction should note it and waits for investment verification", async () => {
        const decision: TransactionDecision =
          transaction.execute(investorVerified);
        ...
        expect(
          decision.stateChange.metadata.investorVerificationStatus
        ).is.equal(InvestorVerificationStatus.Verified);
        expect(
          decision.stateChange.metadata.investmentVerificationStatus
        ).is.equal(InvestmentVerificationStatus.Unverified);
      });
    ...
    describe("And when the investment is verified", () => {
      ...
      it("Then the transaction should request trade disbursement", async () => {
        ...
        expect(
          decision.stateChange.metadata.investmentVerificationStatus
        ).is.equal(InvestmentVerificationStatus.Verified);
      });
    });
  }
);
