...
context(
  "Given the funds transfer was initiated and awaiting for payment",
  () => {
    ...
    describe("When the system notifies about successful payment", () => {
      ...
      it("Then the transaction should request account verification and wait for Broker-Dealer approval", async () => {
        const decision: TransactionDecision =
          transaction.execute(successfulPayment);
    ...
    describe("When the payment failed", () => {
      ...
      it("Then the transaction should be cancelled", async () => {
        const decision: TransactionDecision =
          transaction.execute(failedPayment);
        ...
        expect(decision.stateChange.metadata.unwindReason).is.equal(
          FailureCompletionReason.PaymentFailed
        );
      });
    });
